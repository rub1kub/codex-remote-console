...
- сохраняет SSH/local проекты: один сервер может иметь несколько рабочих папок;
...
Один и тот же `SSH target` можно использовать для нескольких папок, например `/var/www/zavozik.xyz` и `/var/www/hytalemonitoring.com`. В sidebar они будут сгруппированы под одним сервером.

Проекты, настройки и локальные pin-метки чатов лежат в:
