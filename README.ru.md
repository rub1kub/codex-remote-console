...
- Использует одноразовый SSH-пароль только для текущего подключения; пароль не сохраняется.
...
Пароли, OpenAI токены и ChatGPT cookies Codex Remote не хранит.
...
- команда Codex: `codex`
- SSH-пароль: опционально, не сохраняется
...
- Управление MCP/plugin marketplace. Приложение может спросить Codex про MCP/plugins из чата, но install/login/remove пока остаются CLI-сценариями.
- `cloud` и `apply`. Им нужен отдельный UX вокруг удаленных задач и применения patch перед тем, как делать one-click действия.
- Image attachments. Текстовый чат и упоминания файлов уже есть; загрузку изображений нужно делать отдельным attachment-flow.
