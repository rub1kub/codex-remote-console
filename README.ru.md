...
- `mcp`, `plugin`, `cloud`, `apply`: доступны через shortcuts runner команд и slash-команды.
...
- Управление MCP/plugin marketplace, `cloud` и `apply` сначала вынесены через runner команд. Полные мастера можно добавить позже без смены app-server bridge.
- Подписанный auto-update самого приложения не входит в локальную неподписанную сборку.
